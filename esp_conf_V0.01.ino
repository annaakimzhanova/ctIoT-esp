...
const char* mqtt_server2="192.168.137.39"; //linga hotspot

//////Initializes the espClient
void callback(String topic, byte* payload, unsigned int length);
WiFiClient espClient;
PubSubClient client(mqtt_server2,1883,espClient);
...
bool main_network_failed = false;
    ...
    const char* mainnetworkSSID="Netu_Chungi";
    const char* mainnetworkPASS="kozelka1980";

    
    //////////set these parameters acording to cosmic Tech service AP settings////
    const char* cosmicServiceSSID="Nyssa";
    const char* cosmicServicePASS="31206565";
    //////////////////////////////////////////////////////
...
{   connect_retry_flag=0;
    ...
    if(connect_retry_flag==connect_retry_limit)
    {  
      connect_retry_flag1=0;
      main_network_failed = true;
        WiFi.mode(WIFI_OFF);
        delay(2000);
...
void loop()
{
 if(main_network_failed == true)
  { ArduinoOTA.handle();
    if(WiFi.status()!=WL_CONNECTED)
    {setup_wifi();}
  }

 if(main_network_failed == false)
  {
    if(WiFi.status()!=WL_CONNECTED)
    {setup_wifi();}
